...
# Boot-crash hardening (issue #34, kept per #43): a prior release crashed on
# startup with the container reporting cpu=0%/empty logs — whatever failed,
# it failed before a single line reached the log. `set -e` alone gives no
# clue *which* command failed; this trap prints the failing line number +
# command to stderr before the script (and therefore the container) exits, so
# a boot failure can never be silent again. It fires for any command that
# fails under `set -e`, including the `jq` calls below.
